...
class _AuthGateState extends State<AuthGate> {
                          ...
                          if (mode != AuthMode.phone)
                            RichText(
                              text: TextSpan(
                                ...
                                children: [
                                  TextSpan(
                                    text: mode == AuthMode.login ? "Don't have an account? " : 'You have an account? ',
                                  ),
                                  TextSpan(
                                    text: mode == AuthMode.login ? 'Register now' : 'Click to login',
  ...
  Future<void> _handleMultiFactorException(
    ...
    try {
      ...
      final firstTotpHint = e.resolver.hints.firstWhereOrNull((element) => element is TotpMultiFactorInfo);
      ...
      final firstPhoneHint = e.resolver.hints.firstWhereOrNull((element) => element is PhoneMultiFactorInfo);
  ...
  Future<void> _phoneAuth() async {
    if (mode != AuthMode.phone) {
      ...
      if (kIsWeb) {
        final confirmationResult = await auth.signInWithPhoneNumber(phoneController.text);
        final smsCode = await getSmsCodeFromUser(context);
  ...
  Future<void> _signInWithApple() async {
    ...
    if (kIsWeb) {
      ...
      AuthGate.appleAuthorizationCode = userCred.additionalUserInfo?.authorizationCode;
    }
  }
